...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        double encode_tps = num_tokens / (encode_time.count() / 1000.0);
        
        // Benchmark decoding
        auto decode_start = std::chrono::high_resolution_clock::now();
        std::string decoded = tokenizer.decode(ids.ids, false);
        auto decode_end = std::chrono::high_resolution_clock::now();
        auto decode_time = std::chrono::duration_cast<std::chrono::milliseconds>(decode_end - decode_start);
        
        double decode_tps = (decode_time.count() > 0) ? num_tokens / (decode_time.count() / 1000.0) : 0.0;
        ...
        std::cout << "decode_time_ms:" << decode_time.count() << std::endl;
        std::cout << "num_tokens:" << num_tokens << std::endl;
        std::cout << "num_chars:" << num_chars << std::endl;
        std::cout << "tokens_per_sec:" << std::fixed << encode_tps << std::endl;
        std::cout << "decode_tokens_per_sec:" << std::fixed << decode_tps << std::endl;
